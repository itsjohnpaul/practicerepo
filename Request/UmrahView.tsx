...
const DropdownSection = ({
  ...
  return (
    <div className="mb-4 overflow-hidden rounded-lg bg-gray-300 border border-gray-100">
      <button
            ...
            return (
              <>
                <InfoRow
                  ...
                  isVip={data.performer?.type}
                />
                <InfoRow
                  label="Assigned By"
                  value={data.performer?.performerDetail?.assignedby}
                />
                <InfoRow
                  label="Assigned Date"
                  value={data.performer?.performerDetail?.assignedDate}
                />
              </>
            );
          default:
...
/* ---------- TAB: UMRAH STAGES ---------- */
const UmrahStagesTab = () => (
  <div>Umrah stages content</div>
);
    ...
    <div className="min-h-screen bg-[#fcfaf7] p-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-6 flex border-b bg-white border-gray-200">
          {["Basic Details", "Live Tracking", "Umrah Stages"].map(
            (tab) => (
              <button
                key={tab}
        ...
        {/* Tab Content */}
        <div className="w-full bg-white p-6   min-h-screen rounded-lg shadow-sm border border-gray-100">
          {activeTab === "Basic Details" && (
            <BasicDetailsTab data={data} />
          )}
          {activeTab === "Live Tracking" && <LiveTrackingTab />}
          {activeTab === "Umrah Stages" && <UmrahStagesTab />}
        </div>
      </div>
    </div>
  );
